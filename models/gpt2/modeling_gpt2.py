...
@dataclass
class GenerateDecoderOnlyOutputPinal(ModelOutput):
    """
...
class GPT2Attention(nn.Module):
    def __init__(self, config, is_cross_attention=False, layer_idx=None, cross_attention_dim=768):
        super().__init__()
        ...
        if self.is_cross_attention:
            self.c_attn = Conv1D(2 * self.embed_dim, cross_attention_dim)
...
class GPT2Block(nn.Module):
    def __init__(self, config, layer_idx=None):
        ...
        if config.add_cross_attention:
            self.crossattention = GPT2Attention(config, is_cross_attention=True, layer_idx=layer_idx, cross_attention_dim=config.cross_attention_dim)
...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        ###
        # lm_logits[:, :, 2] = lm_logits[:, :, 2] - 1e9 # mask out the stop tokens
        ###
        loss = None
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        log_p = torch.zeros((batch_size, stopping_criteria.max_length), device=input_ids.device) 
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            # Modified by fengyuan: to get the possibility of the choisen token
            next_token_possibilities = probs[range(batch_size), next_tokens]
            log_p[:, input_ids.shape[1] - 1] = next_token_possibilities.log() * unfinished_sequences
